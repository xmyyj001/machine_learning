...
changfac<-function(x){as.numeric(as.character(x))}
...
rm(dat,dat2,ind,skimmed_test,skimmed_train,select_col,changfac)
...
trainRowNumbers <- createDataPartition(dat3$classe, p=0.7, list=FALSE)
...
preProcess_scaldata_model <- preProcess(trainData[,8:59], method='scale')
...
preProcess_scaldata_model <- preProcess(testData[,8:59], method='scale')
preProcess_scaldata_model
...
```{r elastic_net regressiong,cache=TRUE}
library(elasticnet)
elastic_model<-train(y~.,data=trainData1,method="glmnet")
elastic_predictions <- predict(elastic_model,testData)
# Model performance metrics
table(elastic_predictions,testData$classe)
mean(elastic_predictions==testData$classe)
```
We get the reasonable alpha and lambda:
```{r print elastic_model, echo=FALSE}
...
```{r lda and qda model,cache=TRUE}
stopCluster(cl)
num_core<-detectCores(logical = F)
cl <- makePSOCKcluster(num_core)
registerDoParallel(cl)
lda_model<-train(y~.,data=trainData1,method="lda")
predi_lda<-predict(lda_model,testData)
mean(predi_lda==testData$classe)

qda_model<-train(y~.,data=trainData1,method="qda")
predi_qda<-predict(qda_model,testData)
mean(predi_qda==testData$classe)
```
...
```{r random_forests model,cache=TRUE}
rf_model<-train(y~.,data=trainData1,method="rf")
rf_predictions <- predict(rf_model,testData)
table(rf_predictions,testData$classe)
mean(rf_predictions==testData$classe)
```
...
model_results<-resamples(list(ENM=elastic_model,RF=rf_model,LDA=lda_model,QDA=qda_model))
#results<-resamples(list(elastic_model,lda_model,qda_model))
...
The output clearly show the algorithms performed in terms of accuracy and kappa and how consistent has it been. Obviously  the Random forests is the best one.  
We can plot the variables importance.  
...
I will use cross validation to improve the performance of Random forests. As below:

```{r rf_model to forecast,cache=TRUE}
rf_model1<-train(y~.,data=trainData1,method="rf",
...
forecasting_pred<-predict(rf_model1, newdata = forecasting1)
